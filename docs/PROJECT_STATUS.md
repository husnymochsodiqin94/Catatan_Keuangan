...
- [x] **Restyle PWA sesuai mockup (tema terang)**: font Inter, aksen emerald `#10B981` (aksi) + cyan `#06B6D4` (suara/AI), kartu Total Saldo gradient + chips per-akun, kartu "Keluar bln ini + Terbesar", **animasi waveform** saat merekam, dan **layar kunci token** (muncul saat server pakai `CATATAN_TOKEN` / online; lokal tanpa token langsung terbuka). Backend tak berubah; 78 test tetap OK.
