...
- [x] **Foto struk (receipt)**: lampirkan foto ke transaksi — tabel `receipts` (base64 data URI, per user, maks ~1 MB, klien kompres via canvas). Endpoint `POST/GET/DELETE /api/transactions/{id}/receipt`; `has_receipt` di listing. UI: tambah/lihat/hapus di sheet Ubah (kamera), ikon 📎 + tap-lihat di Riwayat. SW `ck-shell-v16`.
- [x] **Split transaction**: 1 pembayaran → beberapa kategori (`create_split`, `POST /api/transactions/split`, min 2 baris); UI sheet "Bagi ke beberapa kategori" dengan hitung sisa live.
- [x] **Kelola kategori sendiri**: kategori kustom di settings, digabung taksonomi (`/api/categories` per-user); layar "Kelola Kategori" di Akun.
- [x] **Mode Gelap & Privacy Blur**: toggle tema gelap (localStorage, diterapkan saat boot) + buramkan nominal (ikon mata Beranda / toggle Akun).
